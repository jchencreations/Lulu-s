...
public class TutorialIngredientManager : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        TutorialGameManager.StartDay.AddListener(DayStart);
    }
    ...
    public void DayStart()
    {
        ...
        foreach (GameObject ing in TutorialGameManager.instance.currentIngredients.Keys)
        {
            Debug.Log(ing.name);
            if (spawnedIngredient.name == ing.name)
            {
                count = TutorialGameManager.instance.currentIngredients[ing];
            }
        }
        if(count == 0)
        {
